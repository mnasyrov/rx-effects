...
import { toSignal, toObservable } from '../rxjs-interop/index';
...
describe('computed()', () => {
  ...
  it.skip('should handle recursion during store updates: Value selector', async () => {
  ...
  // TODO: Not supported
  it.skip('should use a getter with selector', async () => {
    const source = signal({ key: 1, val: 'a' });

    const query = computed(() => source().key);
